...
export default function calculateXY(red: number, green: number, blue: number, model?: string) {
    red = red / 255;
    green = green / 255;
    blue = blue / 255;
    ...
    if (!inReachOfLamps) {
        ...
        if (pAB === null || pAC === null || pBC === null) return null;
...
function colorPointsForModel(model?: string) {
    if (model == null) {
        model = ' ';
    }
...
function checkPointInLampsReach(point: number[], colorPoints: number[][]) {
...
function crossProduct(point1: number[], point2: number[]) {
    return point1[0] * point2[1] - point1[1] * point2[0];
}

function getClosestPointToPoints(pointA: number[], pointB: number[], pointP: number[]) {
...
function getDistanceBetweenTwoPoints(pointA: number[], pointB: number[]) {
